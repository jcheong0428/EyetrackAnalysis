...
%   EyetrackGrapher(FILENAME) Reads data from file FILENAME then plots the
%   sampling rate, number of samples, and interpolation rate along with an
%   interactive graph of gaze location, pupil size, and event, at each
%   sample point. 
%
% Example:
%   EyetrackGrapher('/Users/jinhyuncheong/gamble_auto_2016-02-04_s03_r01.mat.tsv');
%

% for i = 1:length(d.imageArray)
%      newmap=imresize(d.imageArray{i},.5,'nearest'); %need to resize because retina display issue
%      imwrite(newmap,[num2str(d.timeArray(i)),'.png']);
% end
...
x1=[470 670 670 470 470];y1=[395 395 595 595 395];
...
%%
plotbase();
b = uicontrol('Parent',f,'Style','slider','Units','normalized','Position',[0.10 0 0.82 0.20],...
              'value',zeta, 'min',0, 'max',length(xs),'SliderStep',[0.0005 0.01],'Callback',@b_callback);
bgcolor = f.Color;
bl1 = uicontrol('Parent',f,'Style','text','Units','normalized','Position',[0, 0.1, .2,.05],...
                'String','0','BackgroundColor',bgcolor); % for min value
bl2 = uicontrol('Parent',f,'Style','text','Units','normalized','Position',[.81,0.1,.2,.05],...
                'String',mat2str(length(xs)),'BackgroundColor',bgcolor); % for the max value
bl3 = uicontrol('Parent',f,'Style','text','Units','normalized','Position',[.4,.1,.2,.05],...
                'String','Time point','BackgroundColor',bgcolor); % for text explanation

p = uipanel(f,'Title','Data info',...
             'Position',[0.82 0.39  0.1 0.27]);
b1 = uicontrol(p,'Style','pushbutton','String','Raw Data',...
                'Units','normalized',...
                'Position',[.1 .11 .8 .1],'Callback',@b1_callback);
b2 = uicontrol(p,'Style','pushbutton','String','Interpolated Data',...
                'Units','normalized',...
                'Position',[.1 0 .8 .1],'Callback',@b2_callback);
...
b4 = uicontrol(p,'Style','text','Units','normalized','Position',[.1, .52, .8, .1], 'HorizontalAlignment','left',...
                'String',sprintf('Sampling Rate : %2.1f Hz',samplingrate),'BackgroundColor',bgcolor); % Show sampling rate
b5 = uicontrol(p,'Style','text','Units','normalized','Position',[.1, .4, .8, .1],'HorizontalAlignment','left',...
